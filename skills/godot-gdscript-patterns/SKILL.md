...
description: >
  Use when writing, reviewing, or designing GDScript code in Godot 4 —
  implementing nodes/scenes/resources, wiring signals or autoloads, applying
  state machines or component patterns, refactoring scene architecture, or
  evaluating GDScript performance. Also activate on questions framed as
  "how do I do X in GDScript" or "what's the right pattern for Y in Godot".
  Skip for non-code work (level design, art import, editor UI, project
  export config), brainstorming game mechanics, or feature orchestration
  (use godot-feature-workflow for that).
---
...
Two patterns. Pick by node type, not by preference.

**CharacterBody2D** — built-in collision response, engine handles `delta`:
...
**Node2D / Area2D** (projectiles, particles, anything not using a character body) — integrate manually with `delta`:
...
**Frame-independence pitfall:** `position += direction * speed` (no `* delta`) is frame-rate dependent — runs faster on a 144 Hz monitor than 60 Hz. Always multiply by `delta` when integrating manually. `move_and_slide()` already handles `delta` internally — don't multiply by it there.

## Verifying Godot 4 APIs (use the docs MCP)

The patterns below show idiomatic *shape*. The exact API surface — method names, signal signatures, `@export` annotations, enum values, deprecation status — drifts between Godot 4.x minor versions and is what training-data memory most often gets wrong. **Verify before writing. Don't guess from memory.**

### When the docs MCP is required

Reach for it whenever you'd otherwise be guessing:

- Signal signatures (`area_entered`, `body_entered`, `tree_exited`, custom-class signals)
- `@export` / `@export_range` / `@export_group` / `@export_enum` annotations you haven't used recently
- `ResourceLoader`, `FileAccess`, `Tween`, `AnimationPlayer`, physics-server calls
...
3. What to extract (e.g. "signals + argument types only", "deprecation status of `move_and_slide`").
4. A response cap (e.g. "Under 150 words", "5 bullets max").
...
func transition_to(state_name: StringName, msg: Dictionary = {}) -> void:
    ...
    var from_name: StringName = previous_state.name if previous_state else &""
...
```gdscript
# player_idle.gd — consumer: override only the virtuals you need
...
**Convention:** states *write* `player.velocity`; the `Player` script owns `move_and_slide()` in `_physics_process()`. Keeps states physics-agnostic and prevents double-stepping.

### Pattern 2: Autoload Singletons

**Naming:** Autoload scripts should NOT declare `class_name` — the autoload's registered name already becomes a global identifier, and a same-named `class_name` collides ("hides a global script class" error). Reserve `class_name` for scripts you instance directly (e.g. `ObjectPool` in Pattern 4); use plain `extends Node` on autoloaded scripts (as `game_manager.gd` below does).
...
func end_game(won: bool) -> void:
    ...
    if score > high_score:
        high_score = score  # persist via FileAccess if needed
...
```gdscript
# Using resources — duplicate so the shared resource isn't mutated at runtime
...
```gdscript
# pooled_bullet.gd — minimum pool-consumer contract: signal + spawn/despawn hooks + one emit site
...
func on_despawn() -> void:
    pass  # reset bullet-specific state here
